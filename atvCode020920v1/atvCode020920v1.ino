...
void turn_wheels(int channel0)
{
    ...
    if (arr.data[0] > 0)
    {
        //int turn = map(arr.data[0], -1, 0, 1000, 0);
    ...
    if (arr.data[0] < 0)
    {
        //int turn = map(arr.data[0], 0, 1, 0, 1000);
...
void outputGasSignal(int channel2)
{
    if (arr.data[1] > 0)
    {
        mappedOutputValue = map(arr.data[1], 0.05, 1, 45, 214);
        digitalWrite(gasMotorDirection, HIGH);
        analogWrite(gasMotorPwm, mappedOutputValue);
    }
    else if (arr.data[1] < 0)
    {
        mappedOutputValue = map(arr.data[1], -1,-0.05, 45, 214);
